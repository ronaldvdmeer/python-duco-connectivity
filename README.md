...
- `async_set_node_config()` for `PATCH /config/nodes/{node}`; when `parameter` is provided, the typed writer currently supports only `Name`, and accepts sparse payloads built from `PatchConfigNodeValue` leaves or raw API-shaped `{"Val": ...}` objects
