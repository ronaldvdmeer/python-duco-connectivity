"""Tests for local replay sample fixture helpers."""
...
from tests.helpers.replay import (
    BASE_FIXTURE_NAME,
    LOCAL_SAMPLE_FIXTURE_ROOT,
    available_local_sample_profiles,
    build_local_sample_fixture_path,
    build_replay_request,
    load_local_sample_fixture,
...
def test_replay_fixture_roots_use_expected_repository_layout() -> None:
    """Replay helpers should point at the ignored local sample tree."""
    assert LOCAL_SAMPLE_FIXTURE_ROOT == REPLAY_FIXTURE_ROOT / "raw"


def test_build_local_sample_fixture_path_uses_stable_layout() -> None:
...
def test_available_local_sample_profiles_lists_directories(tmp_path: Path) -> None:
    """Profile discovery should return local sample profiles in sorted order."""
    (tmp_path / "focus-v26").mkdir()
    (tmp_path / "silent-v26").mkdir()
    (tmp_path / "README.md").write_text("ignore me", encoding="utf-8")

    assert available_local_sample_profiles(root=tmp_path) == ("focus-v26", "silent-v26")


def test_available_local_sample_profiles_ignores_non_directory_root(
    tmp_path: Path,
) -> None:
    ...
    assert available_local_sample_profiles(root=root_file) == ()
...
def test_replay_helpers_reject_relative_profile_names(profile: str) -> None:
    ...
    with pytest.raises(ValueError, match=r"must not be \. or \.\."):
        build_local_sample_fixture_path(profile, "GET", "/api")

    with pytest.raises(ValueError, match=r"must not be \. or \.\."):
        load_local_sample_fixture_set(profile)
...
def test_replay_helpers_reject_portability_breaking_profile_names() -> None:
    ...
    with pytest.raises(ValueError, match="simple directory name"):
        build_local_sample_fixture_path("silent\\connect-v25", "GET", "/api")

    with pytest.raises(ValueError, match="simple directory name"):
        load_local_sample_fixture_set("silent\\connect-v25")
...
def test_replay_helpers_reject_ambiguous_path_separators(
    ...
    with pytest.raises(ValueError, match=message):
        build_local_sample_fixture_path("silent-connect-v25", "GET", path)


def test_load_local_sample_fixture_returns_single_fixture(tmp_path: Path) -> None:
    """Single fixture loads should preserve the normalized request key."""
    profile_root = tmp_path / "silent-connect-v25" / "GET" / "info"
    profile_root.mkdir(parents=True)
    (profile_root / "module=General;submodule=Board.json").write_text(
        '{"General": {"Board": {"PublicApiVersion": {"Val": "2.5"}}}}',
        encoding="utf-8",
    )

    fixture = load_local_sample_fixture(
        "silent-connect-v25",
        "GET",
        "/info",
        params={"submodule": "Board", "module": "General"},
    ...
    assert fixture.payload == {"General": {"Board": {"PublicApiVersion": {"Val": "2.5"}}}}


def test_load_local_sample_fixture_set_loads_profile_index(tmp_path: Path) -> None:
    """Loading a profile should index each fixture by normalized request."""
    api_root = tmp_path / "silent-connect-v25" / "GET" / "api"
    info_root = tmp_path / "silent-connect-v25" / "GET" / "info"
    nodes_root = info_root / "nodes"
    zones_root = info_root / "zones"

    api_root.mkdir(parents=True)
    info_root.mkdir(parents=True, exist_ok=True)
    nodes_root.mkdir(parents=True)
    zones_root.mkdir(parents=True)

    (api_root / BASE_FIXTURE_NAME).write_text(
        '{"PublicApiVersion": {"Val": "2.5"}}',
        encoding="utf-8",
    )
    (info_root / "module=General;submodule=Board.json").write_text(
        '{"General": {"Board": {"PublicApiVersion": {"Val": "2.5"}}}}',
        encoding="utf-8",
    )
    (nodes_root / BASE_FIXTURE_NAME).write_text('{"Nodes": []}', encoding="utf-8")
    (zones_root / BASE_FIXTURE_NAME).write_text('{"Zones": []}', encoding="utf-8")

    fixture_set = load_local_sample_fixture_set("silent-connect-v25", root=tmp_path)
...
def test_load_local_sample_fixture_set_reports_profile_root_on_missing_profile(
    tmp_path: Path,
) -> None:
...
def test_load_local_sample_fixture_set_reports_conflicting_files(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(ValueError) as err:
        load_local_sample_fixture_set("silent-connect-v25", root=tmp_path)
...
def test_load_local_sample_fixture_set_rejects_invalid_fixture_query_keys(
    tmp_path: Path,
    file_name: str,
    message: str,
) -> None:
...
def test_load_local_sample_fixture_set_rejects_noncanonical_method_directory(
    tmp_path: Path,
) -> None:
